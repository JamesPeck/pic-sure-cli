...
// Every wizard key must exist in the checked-in AIO .env.example contract
// fixture. Update the fixture from pic-sure-all-in-one when that schema changes.
func TestEveryFieldKeyExistsInEnvExample(t *testing.T) {
	path := filepath.Join("..", "..", "testdata", "fixtures", "aio.env.example")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("reading AIO .env.example fixture: %v", err)
	}
